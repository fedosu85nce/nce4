...
def installSystem(storage, bootDev, rootDev, swapDev, callback, diskname, hasMultipath):
    """
    ...
    @type  swapDev: str
    @param swapDev: path to swap device

    @type  callback: InstallationProcess method
    @param callback: method to update progress on screen

    @type  diskname: str
    @param diskname: diskname that is selected by user
    ...
    try:
        ...
        if not isBlockDevice(bootDev):
            raise RuntimeError('Boot partition [%s] is not valid' % bootDev)

        # given root device is not valid: error
        if not isBlockDevice(rootDev):
            raise RuntimeError('Root partition [%s] is not valid' % rootDev)

        # given swap device is not valid: error
        if not isBlockDevice(swapDev):
            raise RuntimeError('Swap partition [%s] is not valid' % swapDev)
        ...
        cmd = "mount %s %s 2>/dev/null" % (bootDev, bootPath)
        rc = subprocess.call(cmd, shell=True)
        ...
        updateFstabFile(rootDir, bootDev, rootDev, swapDev)
        ...
        # update multipath
        if hasMultipath:
            setupMultipath(bootDev, rootDev, rootDir)

        # the zipl bootloader should be generated at the last step of the installation
        installBootloader(storage, rootDir, rootDev, bootDev, swapDev, diskname)
...
def setupMultipath(bootDev, rootDev, rootDir):
    """
    Setups multipath accordingly
...
def installBootloader(storage, rootDir, rootDev, bootDev, swapDev, diskname):
    """
    ...
    @type  rootDev:  string
    @param rootDev:  path to root device that is created

    @type  bootDev:  string
    @param bootDev:  path to boot device that is created

    @type  swapDev:  string
    @param swapDev:  path to swap device that is created

    @type  diskname: string
    @param diskname: diskname that is selected for installation
    ...
    boot_args.set_args("/proc/cmdline", storage, rootDev, bootDev, swapDev, diskname)
    ...
    cmd = 'chroot %s zipl >>/etc/ipl.log 2>&1' % (rootDir)
    if subprocess.call(cmd, shell=True) != 0:
    ...
    bootdisk = storage.devicetree.getDeviceByName(diskname)
    cmd = 'chroot %s chreipl %s >>/etc/ipl.log 2>&1' % (rootDir, bootdisk.path)
    if subprocess.call(cmd, shell=True) != 0:
...
def updateFstabFile(rootDir, bootDev, rootDev, swapDev):
    """
    ...
    content.append("%s    /        ext4    defaults    1 1\n" % rootDev)
    content.append("%s    /boot    ext4    defaults    1 2\n" % bootDev)
