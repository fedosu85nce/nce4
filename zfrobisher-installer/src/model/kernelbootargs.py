...
class kernelBootArgs(object):
    ...
    def set_args(self, cmdline_file, storage, rootDev, bootDev, swapDev, diskname):
        ...
        if 'mpath' in diskname:
            bootdisk = bootdisk.parents[1]
        if not self.is_valid_boot_dev(bootdisk):
            logger.error("%s is not a valid boot device" % (bootdisk.name))
            raise RuntimeError('Error running installBootloader')
        self.__args = self.__args | bootdisk.dracutSetupArgs()
        rootdevice = storage.devicetree.getDeviceByPath(rootDev)
        self.__args = self.__args | rootdevice.dracutSetupArgs()
        self.__args.add("root=%s" % rootdevice.path)
        bootdevice = storage.devicetree.getDeviceByPath(bootDev)
        self.__args = self.__args | bootdevice.dracutSetupArgs()
        swapdevice = storage.devicetree.getDeviceByPath(swapDev)
        self.__args = self.__args | swapdevice.dracutSetupArgs()
