...
#from modules.partitioner.partitioner import Partitioner
from viewer.newt.partitioncfg.partitioner import Partitioner
from viewer.newt.partitioncfg.partition_interface import INDEX_BACK
...
class Controller(object):
    ...
    def diskSelection(self):
        ...
        try:
            ...
            while True:
                ...
                diskSelected = self.__viewer.getDiskSelection(self.__partitioner,lunset).run()

                # no disk selected or back pressed: go back to main menu
    ...
    def installProgress(self, diskSelected):
        ...
        if self.doPartitioning(diskSelected) == INDEX_BACK:
            return 'back'
        ...
        boot = self.__partitioner.boot.path
        root = self.__partitioner.root.path
        swap = self.__partitioner.swap.path

        self.__data['model'].insert('bootDevice', boot)
        self.__data['model'].insert('rootDevice', root)
        ...
        try:
            ...
            installSystem(self.__partitioner.storage, boot, root, swap, install.installInfo, diskSelectedStr,
    ...
    def doPartitioning(self, diskSelected):
        partitioning = self.__viewer.getPartitioning(self.__partitioner, diskSelected)
