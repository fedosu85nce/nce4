...
class SelectVg:
    ...
    def run(self):
        ...
        vg = self._vgList.current()
        # Delete current VG if exist
        if rc == PART_BUTTON_DEL.localize():
        ...
        if rc == PART_BUTTON_OK.localize():
            if vg is None:
                ...
                # Update partition with this vg information
                return PART_BUTTON_OK.localize()
            else:
                # Delete old LVM device if exist
                # Create LVM device using this vg
                # Update partition with this vg information
                return PART_BUTTON_OK.localize()
        if rc == PART_BUTTON_BACK.localize():
            return PART_BUTTON_BACK.localize()
        return rc
